...
export const getDefectosxid = async (req, res) => {
  try {
    const [result] = await conmysql.query(`
      SELECT d.*, l.lote_codigo, l.lote_libras_remitidas, p.proveedor_nombre, u.usuario_nombre
      FROM defectos d
      LEFT JOIN lote l ON d.lote_id = l.lote_id
      LEFT JOIN proveedor p ON l.proveedor_id = p.proveedor_id
...
export const postDefectos = async (req, res) => {
  try {
    ...
    const [nuevoDefecto] = await conmysql.query('SELECT * FROM defectos WHERE defectos_id=?', [nuevoId]);

    global._io.emit("defecto_nuevo", nuevoDefecto[0]);

    res.json({ id: nuevoId, message: "Defectos registrados con éxito", codigo });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

// PUT: Actualizar completo (full replacement, explicit fields for consistency)
export const putDefectos = async (req, res) => {
  try {
    ...
    const {
      ...
      lote_id, usuario_id, defectos_codigo
    } = req.body;
    ...
    const [result] = await conmysql.query(
      ...
      [
        ...
        defectos_porcentaje_falta_de_corte, defectos_porcentaje_corte_profundo, defectos_porcentaje_corte_desviado,
        defectos_basura, defectos_total_defectos, defectos_observaciones, defectos_acciones_correctivas,
        lote_id, usuario_id, defectos_codigo, id
      ]
    );
...
// PATCH: Actualización parcial (dynamic, with protected fields filtered)
