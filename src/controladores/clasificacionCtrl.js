...
// GET: Todas las clasificaciones with JOINs
...
/* export const getClasificacionxid = async (req, res) => {
  try {
    ...
    `, [req.params.id]);
    if (result.length <= 0) return res.status(404).json({ clasificacion_id: 0, message: "Clasificación no encontrada" });
    res.json(result[0]);
  } catch (error) {
...
// GET: Obtener todos ordenados descendente por fecha/hora
export const getClasificacion = async (req, res) => {
  try {
    const [result] = await conmysql.query(`
      SELECT c.*, l.lote_codigo, u.usuario_nombre, pr.proveedor_nombre, t.tipo_descripcion,
        cl.clase_descripcion, co.color_descripcion, cr.corte_descripcion, ta.talla_descripcion, 
        pe.peso_descripcion, g.glaseo_cantidad, p.presentacion_descripcion, o.orden_codigo, 
        m.maquina_descripcion, gr.grupo_nombre, ch.coche_descripcion
      FROM clasificacion c
      LEFT JOIN lote l ON c.lote_id = l.lote_id
      LEFT JOIN usuario u ON c.usuario_id = u.usuario_id
      LEFT JOIN proveedor pr ON c.proveedor_id = pr.proveedor_id
      ...
      LEFT JOIN grupo gr ON c.grupo_id = gr.grupo_id
      LEFT JOIN coche ch ON c.coche_id = ch.coche_id
      ORDER BY c.clasificacion_coche_fecha DESC, c.clasificacion_coche_hora DESC
    `);
    res.json(result);
  } catch (error) {
    return res.status(500).json({ message: "Error al consultar Clasificación" });
  }
};
...
// POST: Insert, calc rendimiento, update orden_libras_pendientes
export const postClasificacion = async (req, res) => {
  try {
    ...
    // Update orden_libras_pendientes
    const [sumClasif] = await conmysql.query('SELECT SUM(clasificacion_total) as sum_total FROM clasificacion WHERE orden_id = ?', [orden_id]);
    const sumTotal = sumClasif[0].sum_total || 0;
    await conmysql.query('UPDATE orden SET orden_libras_pendientes = orden_total_libras - ? WHERE orden_id = ?', [sumTotal, orden_id]);
...
// PUT: Update, recalc rendimiento, update pending
